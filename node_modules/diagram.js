function diagramInit(loadFile){
  ...
  myDiagram.toolManager.resizingTool.doMouseUp = function() {
      ...
      var elementWidth = document.getElementById("width" + node.key);
      var elementHeight = document.getElementById("height" + node.key);

      myDiagram.model.startTransaction("Resize node " + node.key);
      myDiagram.model.setDataProperty(node.data, "width", node.width);
      elementWidth.value = node.width * 20;
      myDiagram.model.setDataProperty(node.data, "height", node.height);
      elementHeight.value = node.height * 20;
    ...
    myDiagram.toolManager.draggingTool.doMouseUp = function(){
      var parts = this.draggingParts;
      parts.iterator.each(function(part){
        if (part.data.loc.split(" "[0] == 0)) return;
        var elementFrontOffset = document.getElementById("frontOffset" + part.data.key);
        var elementBackOffset = document.getElementById("backOffset" + part.data.key);
        elementFrontOffset.value = part.location.y * 20;
        // elementBackOffset.value = parseInt(elementBackOffset.value) + (part.location.y * 20);
      });
